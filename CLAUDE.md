...
- **Client sync has one observation owner and no synthetic lifecycle.** DefraDB's strict P2P status is adapted once in `gents::p2p_observability`; `ClientSyncStateOwner` combines that observation with transport and configured-peer facts; `project_sync_health` derives the four product states; the bridge transports that projection; and `projectSyncOperationalStatus` supplies UI copy/actions. `AgentBehaviorReadiness` owns runtime process/admission readiness only—it is never a database-sync source. Do not recreate `stalled`, elapsed-clock heuristics, pairing-derived global sync, optional wire defaults, or another UI-local projector. See `docs/superpowers/specs/2026-08-17-mobile-session-sync-design.md` for the compact ownership contract.
- **External code is held at arm's length:** rig-core is the provider/streaming client only ("Layer A"); native messages cross through `llm::rig_compat`, while `provider_input` is the narrow wire-projection seam used to account the exact provider DTO. The message family is native and byte-compatible with what's persisted (`gents-protocol::message`). Full rig removal is staged (#438/#439). DefraDB itself comes from the public `sourcenetwork/defradb.rs` repository, pinned in the workspace `Cargo.toml` — look there when node, schema, or identity behavior is in question. Claude subscription seats are reached over Anthropic Messages HTTP with an agent-scoped `OAuthCredential` written by `gents claude-login` and refreshed by gents; the `claude` binary is not a dependency.
